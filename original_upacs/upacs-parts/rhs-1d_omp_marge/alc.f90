...
contains
  ...
  subroutine rhs(rho,u,v,w,pre,vis, &
    ...
    real(8) :: fmass_t,fmomx_t,fmomy_t,fmomz_t,fene_t


    flop = 170+153 + 132+54 + 15 + 5 ! convective + viscous + update + marge
    byte = 25+15 + 68+21 + 20 + 15
...
#include "../bodies/vflux_body_1d.inc"
      fmass(l) = fmass_t + fmass(l) 
      fmomx(l) = fmomx_t + fmomx(l) 
      fmomy(l) = fmomy_t + fmomy(l) 
      fmomz(l) = fmomz_t + fmomz(l) 
      fene(l)  = fene_t  + fene(l) 
      !
    enddo
    !$omp end do
    !
