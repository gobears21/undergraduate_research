       ...
       Matrix* p,Matrix* bnd,Matrix* wrk1,Matrix* wrk2)
{
  ...
  kmax= p->mdeps-1;
  {
    for(n=0 ; n<nn ; n++){
      ...
      #pragma omp parallel for shared(a,p,b,c,bnd,wrk1,wrk2,imax,jmax,kmax,omega) private(i,j,k,s0,ss) reduction(+:gosa)
      for(i=1 ; i<imax; i++)
        for(j=1 ; j<jmax ; j++)
          for(k=1 ; k<kmax ; k++){
      ...
      #pragma omp parallel for shared(p,wrk2,imax,jmax,kmax) private(i,j,k)
      for(i=1 ; i<imax ; i++)
        for(j=1 ; j<jmax ; j++)
          for(k=1 ; k<kmax ; k++)
