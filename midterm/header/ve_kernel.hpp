...
namespace cl::sycl::detail
{
  ...
  struct VEKernel : public Kernel
  {
    ...
    vector_class<uint64_t> copy_in(struct veo_args *argp)
    {
       ...
       * *******************************/
       /*************
        // calculate the current bank id
        if ((ve_addr_int % CELL_SIZE) == 0)
        {
          //printf("if %lu %d\n",ve_addr_int,i);
          current_bank_id = (ve_addr_int / CELL_SIZE) % BANK_NUMBER;
          //printf("array %d current bank id is %d\n", i, current_bank_id);
        }
        else // ( (ve_addr_int % CELL_SIZE != 0))
        {
          //printf("else %lu %d\n",ve_addr_int,i);
          current_bank_id = (ve_addr_int / CELL_SIZE) % BANK_NUMBER + 1;
          //printf("array %d current bank id is %ld\n", i, current_bank_id);
          //fflush(stdout);
        }
        ...
        **************/
        ...
        //long log2i = log(i) / log(2);
        //long numerator = BANK_NUMBER * (2*(i % (long)pow(2, log2i))+1);
        //long denominator = pow(2, log2i+1);
        //priority_and_bank_id[i] = numerator/denominator;
