  ...
  nec::VEContext ctx;
    ...
    /***
        uint64_t a_int, b_int;
   	    // allocation on VE
        vec_a = (double *)veo_alloc_mem(proc.ve_proc, &a_int,sizeof(double)*input_size);   //veo_alloc_mem(proc.ve_proc, &vec_a,sizeof(double)*input_size) what is the proc.ve_proc ??
        vec_b = (double *)veo_alloc_mem(proc.ve_proc, &b_int,sizeof(double)*input_size); 
    ***/
