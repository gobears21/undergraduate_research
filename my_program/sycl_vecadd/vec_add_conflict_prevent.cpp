...
#define BANKNUMBER 16
#define CELLSIZE 128   // in byte
...
{
	long unsigned int input_size, pedding;   
	...
	vec_a = (double *)malloc(sizeof(double)*input_size+CELLSIZE*BANKNUMBER);
	vec_b = (double *)malloc(sizeof(double)*input_size+CELLSIZE*BANKNUMBER);
	
	// finding d
	d = ((vec_a-vec_b)/CELLSIZE)) % BANKNUMBER;
	
	if( 480<(d%512) && (d%512) > 544)
	{
		pedding=256;
	}
	...
	{           // By sticking all the SYCL work in a {} block, we ensure all SYCL tasks must complete before exiting the block, because the destructor of resultBuf will wait.
        ...
        one.submit([&](handler &cgh) {        // Create a command_group to issue commands to the queue    
