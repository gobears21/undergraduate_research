...
class DataContainerND<T, 2> : public DataContainer {
 ...
 public:
  explicit DataContainerND(const cl::sycl::range<2> &r) : data_range(r), need_deallocate(true) {
  ...
  explicit DataContainerND(T *data, const cl::sycl::range<2> &r) : data_range(r), need_deallocate(false) {
  ...
  explicit DataContainerND(T **data, const cl::sycl::range<2> &r) : data_range(r), need_deallocate(false) {
    ...
    for (size_t i = 0; i < x; i++) {
      ptr[i] = data[i];
      // std::cout << "data container 2D ptr: " << ptr[i] << std::endl;
    }
    access_ptr = ptr;
  }
